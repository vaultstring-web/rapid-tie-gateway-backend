...
async function getFinanceOfficer(req, next) {
    if (!req.user) {
        next(new errorHandler_1.AppError('Unauthorized', 401));
        return null;
    }
    const officer = await server_1.prisma.financeOfficer.findUnique({
        where: { userId: req.user.id },
        include: { organization: true },
    });
    if (!officer) {
        next(new errorHandler_1.AppError('Finance officer profile not found', 404));
        return null;
    }
    return officer;
}
...
async function parseCsvBuffer(buffer) {
    return new Promise((resolve, reject) => {
        (0, csv_parse_1.parse)(Buffer.from(buffer).toString('utf-8'), { columns: true, skip_empty_lines: true, trim: true }, (error, records) => {
            if (error) {
                reject(error);
                return;
            }
...
async function parseUpload(file) {
    ...
    throw new errorHandler_1.AppError('Only .csv and .xlsx files are supported', 400);
}
async function eventInsightsForRequests(requests) {
    const perRequestEvents = await Promise.all(requests.map((request) => server_1.prisma.event.findMany({
        where: {
...
class FinanceController {
    async getDashboard(req, res, next) {
        try {
            ...
            const [pendingDisbursements, approvedNotDisbursed, activeBatches, completedBatches, budgets, recentBatches, paidRequests,] = await Promise.all([
                server_1.prisma.dsaRequest.count({
                    where: {
                ...
                server_1.prisma.disbursementBatch.count({
                ...
                server_1.prisma.budget.findMany({
                    where: { organizationId: orgId },
                    include: { department: { select: { name: true } } },
                    orderBy: { fiscalYear: 'desc' },
                }),
                server_1.prisma.disbursementBatch.findMany({
                    where: { organizationId: orgId },
            ...
            const utilizationRate = totalAllocated > 0
                ...
                : 0;
            const budgetAlerts = budgets.filter((b) => b.allocated > 0 && (b.spent / b.allocated) >= 0.8);
    ...
    async getBudgets(req, res, next) {
        try {
            ...
            const where = { organizationId: officer.organizationId };
            if (fiscalYear)
                where.fiscalYear = fiscalYear;
            const budgets = await server_1.prisma.budget.findMany({
                where,
    ...
    async getDisbursements(req, res, next) {
        try {
            const officer = await getFinanceOfficer(req, next);
            if (!officer)
                return;
            ...
            const [requests, total, sumAgg] = await Promise.all([
                ...
                server_1.prisma.dsaRequest.aggregate({ where, _sum: { totalAmount: true } }),
            ]);
            const rows = await Promise.all(requests.map(async (request) => {
                ...
                const mobileMoney = request.employee.mobileMoney;
                const bankAccount = request.employee.bankAccount;
                const recipientValid = Boolean((mobileMoney && mobileMoney.phoneNumber) ||
    ...
    async getBatches(req, res, next) {
        try {
            ...
            const [batches, total] = await Promise.all([
                ...
                server_1.prisma.disbursementBatch.count({ where }),
            ]);
    ...
    async getBatch(req, res, next) {
        try {
            ...
            if (!officer)
                return;
            const { id } = req.params;
            const batch = await server_1.prisma.disbursementBatch.findFirst({
            ...
            if (!batch)
                return next(new errorHandler_1.AppError('Batch not found', 404));
    ...
    async createBatch(req, res, next) {
        try {
            ...
            if (!requestIds || !Array.isArray(requestIds) || requestIds.length === 0) {
                return next(new errorHandler_1.AppError('requestIds array is required and must not be empty', 400));
            }
            const requests = await server_1.prisma.dsaRequest.findMany({
                where: {
                    id: { in: requestIds },
                    organizationId: officer.organizationId,
                    status: 'APPROVED',
            ...
            if (requests.length !== requestIds.length) {
                ...
                return next(new errorHandler_1.AppError(`Some requests are invalid, already batched, or not approved: ${missing.join(', ')}`, 400));
            }
            const totalAmount = requests.reduce((sum, r) => sum + r.totalAmount, 0);
            const batchNumber = generateBatchNumber();
            const batch = await server_1.prisma.$transaction(async (tx) => {
                ...
                await Promise.all(requests.map((r) => {
                    ...
                    const mobileMoney = r.employee.mobileMoney;
                    const bankAccount = r.employee.bankAccount;
    ...
    async processBatch(req, res, next) {
        try {
            ...
            if (batch.status === 'completed') {
                return next(new errorHandler_1.AppError('Batch is already completed', 400));
            }
            ...
            if (status === 'completed') {
                ...
                await server_1.prisma.disbursementBatch.update({ where: { id }, data: updateData });
            }
            const updated = await server_1.prisma.disbursementBatch.findUnique({ where: { id } });
    ...
    async uploadBulkDisbursement(req, res, next) {
        try {
            const officer = await getFinanceOfficer(req, next);
            if (!officer)
                return;
            const file = req.file;
            if (!file) {
                return next(new errorHandler_1.AppError('Spreadsheet file is required', 400));
            }
            const rows = await parseUpload(file);
            if (rows.length === 0) {
                return next(new errorHandler_1.AppError('No rows found in uploaded file', 400));
            }
            ...
            if (validRows.length === 0) {
                return next(new errorHandler_1.AppError('All rows are invalid; no batch created', 400));
            }
                ...
                await Promise.all(validRows.map(({ row }) => {
                    ...
                    return tx.disbursementItem.create({
                        data: {
                            ...
                            recipientPhone: row.recipientPhone || mobileMoney?.phoneNumber || null,
                            recipientAccount: row.recipientAccount || bankAccount?.accountNumber || null,
