// controllers/qrcode.controller.ts
...
const emailQueue: Array<{ ticketId: string; email: string; qrImage: string; attendeeName: string; eventName: string }> = [];
...
async function sendTicketEmail(item: any): Promise<void> {
  console.log(`📧 Sending ticket email to ${item.email} for ${item.eventName}`);
  console.log(`   QR Code: ${item.qrImage.substring(0, 100)}...`);
}

// Update delivery status
async function updateDeliveryStatus(ticketId: string, status: string, error?: string) {
...
// Regenerate QR codes for event
export const regenerateEventQRCodes = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const user = req.user;
    if (!user) {
    ...
    const result = await QRCodeService.regenerateEventQRCodes(eventId, role);
...
// Regenerate single ticket QR code
export const regenerateTicketQRCode = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const user = req.user;
    if (!user) {
      res.status(401).json({ success: false, message: 'Authentication required' });
    ...
    const result = await QRCodeService.regenerateQRCode(ticketId, role);
...
// Generate role-specific QR codes for event
export const generateRoleSpecificQRCodes = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const user = req.user;
    if (!user) {
      res.status(401).json({ success: false, message: 'Authentication required' });
    ...
    for (const role of roles) {
      ...
      for (const ticket of tickets) {
        const { qrCode, signature, qrImage } = await QRCodeService.generateRoleSpecificQRCode(
          ticket.id,
          role,
          permissions
        );
...
//function getMetadataValue(metadata: any, key: string): any {
 // if (!metadata || typeof metadata !== 'object') return null;
  //return metadata[key];
//}

// Get delivery status for event QR codes
...
export const queueBulkEmails = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const user = req.user;
    if (!user) {
      res.status(401).json({ success: false, message: 'Authentication required' });
