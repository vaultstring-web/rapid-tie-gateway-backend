...
export class OrganizerController {
  ...
  // ─── Helper ──────────────────────────────────────────────────────────────────
  private async getOrganizer(req: AuthRequest, next: NextFunction) {
    if (!req.user) { next(new AppError('Unauthorized', 401)); return null; }
    const organizer = await prisma.eventOrganizer.findUnique({
      where: { userId: req.user.id },
    });
    if (!organizer) { next(new AppError('Organizer profile not found', 404)); return null; }
    return organizer;
  }

  // ======================
  // 📊 Dashboard (summary stats + event lists)
  // ======================
  async getDashboard(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // Get events with counts (more efficient than loading all nested data)
  ...
  // ======================
  // 📋 Event list (paginated) — fixed from dashboard-style response
  // ======================
  ...
  async createEvent(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const {
        name, description, shortDescription, category, type,
        venue, city, country, amount, startDate, endDate,
        timezone, capacity, coverImage, images, visibility,
      } = req.body;
      ...
      if (images && typeof images === 'string') {
        ...
        catch { return next(new AppError('Images must be valid JSON array', 400)); }
      }
  ...
  async updateEvent(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const { name, description, shortDescription, category, type, venue, city, country,
        amount, startDate, endDate, timezone, capacity, coverImage, images, visibility, status } = data;
      ...
      if (status && status !== existingEvent.status) {
        ...
        if (!allowed?.includes(status)) {
          return next(new AppError(`Invalid status transition from ${existingEvent.status} to ${status}`, 400));
        }
      }
  ...
  async deleteEvent(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user?.organizer?.id) {
        return next(new AppError('Only organizers can delete events', 403));
      }
      ...
      if (!['DRAFT', 'CANCELLED'].includes(event.status)) {
        return next(new AppError('Only DRAFT or CANCELLED events can be deleted. Cancel a published event first.', 400));
      }
  ...
  async updateProfile(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const {
        ...
        organizationName, organizationType, contactPerson, website, logo,
      } = req.body;
